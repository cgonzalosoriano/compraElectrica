...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, contractId, reason, subject, message } = await request.json();

    if (!action || !contractId) {
      return NextResponse.json({ error: 'Acción y ID de contrato son requeridos' }, { status: 400 });
    }
    ...
    switch (action) {
      case 'reject':
        ...
        // Crear un mensaje sobre la cancelación
        if (reason) {
    ...
    console.error('Error en acción de contrato:', error);
    return NextResponse.json(
