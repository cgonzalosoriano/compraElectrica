  ...
  try {
    ...
    <body>
        ...
        <div class="section">
            ...
            <div class="clause">
                ...
                <p>CUIT: ${clientOffer.offer?.provider?.cuit || 'N/A'}</p>
                <p>Email: ${clientOffer.offer?.provider?.email}</p>
                <p>Dirección: ${clientOffer.offer?.provider?.address || 'N/A'}</p>
            </div>

            <div class="clause">
                <p><strong>CLIENTE:</strong></p>
                <p>Razón Social: ${clientOffer.client?.companyName || clientOffer.client?.name}</p>
                <p>CUIT: ${clientOffer.client?.cuit || 'N/A'}</p>
                <p>Email: ${clientOffer.client?.email}</p>
                <p>Dirección: ${clientOffer.address || clientOffer.client?.address || 'N/A'}</p>
                <p>Usuario Distribuidora: ${clientOffer.userNumber || 'N/A'}</p>
                <p>Distribuidora: ${clientOffer.distributorName || 'N/A'}</p>
                <p>Región: ${clientOffer.region || 'N/A'}</p>
                <p>Tipo de Tarifa: ${clientOffer.tariffType || 'N/A'}</p>
            </div>
        </div>
        ...
        <div class="section">
            ...
            <table class="terms-table">
                ...
                <tr>
                    <td>Nodo de Entrega</td>
                    <td>${clientOffer.offer?.deliveryNode || 'N/A'}</td>
                    <td>-</td>
                </tr>
                <tr>
                    <td>Fuente de Generación</td>
                    <td>${clientOffer.offer?.generationSource || 'N/A'}</td>
                    <td>-</td>
                </tr>
            </table>
        </div>
        ...
        <div class="section">
            ...
            <div class="clause">
                ...
                <p>${finalTerms.paymentTerms || 'No especificado'}</p>
            </div>

            <div class="clause">
                <p><strong>Garantías:</strong></p>
                <p>${finalTerms.guarantees || 'No especificado'}</p>
            </div>

            ${finalTerms.otherConditions ? `
            <div class="clause">
                <p><strong>Otras Condiciones:</strong></p>
                <p>${finalTerms.otherConditions}</p>
            </div>
            ` : ''}
        </div>

        <div class="section">
            <h3>CLAUSULAS GENERALES</h3>
            
            <div class="clause">
                <p><strong>1. OBJETO:</strong> El presente contrato tiene por objeto el suministro de energía eléctrica en el Mercado Eléctrico Mayorista de Argentina, bajo las condiciones técnicas y comerciales aquí establecidas.</p>
            </div>
            
            <div class="clause">
                <p><strong>2. VIGENCIA:</strong> El presente contrato tendrá vigencia por ${finalTerms.term} meses, contados a partir de la fecha de firma del presente documento.</p>
            </div>
            
            <div class="clause">
                <p><strong>3. REGULACIÓN:</strong> Las partes se someten a la regulación del Mercado Eléctrico Mayorista y a las disposiciones de CAMMESA (Compañía Administradora del Mercado Mayorista Eléctrico).</p>
            </div>
            
            <div class="clause">
                <p><strong>4. FACTURACIÓN:</strong> La facturación se realizará de acuerdo con los términos de pago establecidos y las mediciones del sistema de CAMMESA.</p>
            </div>
            
            <div class="clause">
                <p><strong>5. INCUMPLIMIENTO:</strong> En caso de incumplimiento de cualquiera de las partes, se aplicarán las penalidades establecidas por la regulación del Mercado Eléctrico Mayorista.</p>
            </div>
        </div>
        ...
        <div class="signature-section">
            <div class="signature-box">
                ...
                <p>${clientOffer.offer?.provider?.companyName}</p>
            ...
            <div class="signature-box">
                ...
                <p>${clientOffer.client?.companyName}</p>
                <p>Fecha: _______________</p>
            </div>
        </div>
    </body>
    ...
    if (existingContract) {
      await prisma.contract.update({
        ...
        data: { 
          terms: contractHTML,
          status: 'NEGOTIATING'  // Cambiar a un estado válido
        }
      });
    } else {
    ...
    return NextResponse.json({ 
      ...
      downloadUrl: `/api/contracts/${contractId}/download-pdf`
    });
