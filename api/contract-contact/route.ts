...
export async function POST(request: NextRequest) {
  try {
    ...
    const { subject, message, contractId } = await request.json();

    if (!subject || !message || !contractId) {
      return NextResponse.json({ error: 'Asunto, mensaje y ID de contrato son requeridos' }, { status: 400 });
    }
